...
public class MainMenuController {
    ...
    private ObservableList<Appointment> reportsTypeAndMonth = FXCollections.observableArrayList();

    private ObservableList<Customer> customerDivisionReport = FXCollections.observableArrayList();

    private ObservableList<Appointment> contactScheduleReport = FXCollections.observableArrayList();
    ...
    /**
     * {@return selected <code>Customer</code>} Used by modify pages
     ...
     */
    public void constructCustomerTable() throws SQLException {
        try {
            Statement statement = JDBC.getConnection().createStatement();
            String query = "Select * from customers Order By Customer_ID";
            ResultSet rs = statement.executeQuery(query);
            while (rs.next()) {
                Customer cust = new Customer(rs.getInt("Customer_ID"), rs.getString("Customer_Name"),
                        rs.getString("Address"), rs.getString("Postal_Code"), rs.getString("Phone"),
                        rs.getInt("Division_ID"));
                cust.setCountryId(cust.getDivisionId());
                cust.setDivisionName(cust.getDivisionId());
     ...
     * @throws SQLException if error occurs during database connection or query execution.
     */
    public void constructAppointmentTable() throws SQLException {
        try {
            Statement statement = JDBC.getConnection().createStatement();
            String query = "Select * from appointments Order By Appointment_ID";
            ResultSet rs = statement.executeQuery(query);
            while (rs.next()) {
                Appointment appt = new Appointment(rs.getInt("Appointment_ID"), rs.getString("Title"),
                        rs.getString("Description"), rs.getString("Location"), rs.getInt("Contact_ID"),
                        rs.getString("Type"), Timestamp.valueOf(rs.getString("Start")),
                        Timestamp.valueOf(rs.getString("End")), rs.getInt("Customer_ID"), rs.getInt("User_ID"));
                appt.setContactName(appt.getContactId());
    ...
    public void filterAppointments() {
        ...
        allAppointments.stream().filter(appt -> appt.getStartDateTime()  //LAMBDA EXPRESSION is used here for increased code readability.
                .before(month) && appt.getStartDateTime()//Without the lambda expression, the
                .after(now))                             //code would consist of if statements crowding the function, reducing readability.
    ...
    public void initialize() throws SQLException {
        ...
        appointmentWithin15MinutesText();
